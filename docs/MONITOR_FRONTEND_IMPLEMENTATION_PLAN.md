...
MVP の新フロントエンドは、第一階層を `ダッシュボード` に集約します。`回答品質`、`連続質問分析`、`ユーザー一覧` は独立ページではなく、ダッシュボード内セクションとして実装します。深いユーザー管理、会話確認、メッセージ確認、ユーザー単位の品質分析は `ユーザー一覧` の `詳細` から `ユーザー詳細` へドリルダウンして表示します。

| フロントエンド表示領域 | API | 扱い |
| --- | --- | --- |
| `ダッシュボード` | `GET /api/metrics/system-dashboard` | KPI、利用推移、活性度、環境・モード、回答品質、追問を表示する。 |
| `ユーザー一覧` | `GET /api/metrics/users` | ダッシュボード下部のユーザー一覧として表示する。 |
| `ユーザー詳細` | `GET /api/metrics/users/{user_id}` | ユーザー一覧から `詳細` で `/dashboard?user_id={user_id}` へ遷移する。 |
...
ダッシュボード上部のグローバル期間選択では `カスタム` は表示せず、`今日`、`直近6時間`、`直近12時間`、`過去3日`、`過去7日`、`過去14日`、`過去30日`、`過去60日`、`全部` を表示します。`カスタム` は主に export dialog と API 互換用です。
...
`利用推移` と `活性度分布` はグローバル期間とは別に、セクション内の `表示期間` で `3日`、`7日`、`14日`、`30日`、`全部` を選択します。初期値はどちらも `7日` です。`ユーザー一覧` は常に直近7日列口径で表示し、グローバル期間の影響を受けません。
...
`連続質問分析` も独立ページではなく、ダッシュボード内セクションとして表示します。
...
| F2-8 | ユーザー一覧を実装 | 同上 | 指定列、活性度 filter、検索、`詳細` が動く。 |
...
`回答品質分析`、`連続質問分析`、`ユーザー一覧` はダッシュボード内セクションです。`ユーザー詳細` は nav に置かず、`ユーザー一覧` の `詳細` から遷移します。`チャット記録確認` と `データ健全性` は独立フロントエンドページにしません。
